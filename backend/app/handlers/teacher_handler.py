...
def handle_get_student_submissions(staff_id, course_id, homework_id):
    """获取学生作业提交列表（支持教师和助教）"""
    try:
        course_id = int(course_id)
...
def handle_grade_submission(staff_id, submission_id, grade_data):
    """处理作业批改（支持教师和助教）"""
    try:
...
def handle_add_ta_to_course(teacher_id, course_id, data):
    ...
    success, result = add_ta_to_course(course_id, staff_no)
    if success:
        return jsonify({
            "message": "助教添加成功",
            "ta": result
...
def handle_publish_homework_grades(staff_id, course_id, homework_id):
    """发布某次作业的成绩"""
